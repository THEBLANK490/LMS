...
const CourseDetail = () => {
  ...
  const enrollCourse = async () => {
    const courseInstance = courseData.id;
    console.log(courseInstance);
    const formData = new FormData();
    formData.append('course', parseInt(courseData.id));
  ...
  const markAsFav = () => {
    ...
    try {
      axios.post(baseUrl + '/student-add-favorite-course/', formData, {
        ...
        if (res.status === 200 || res.status === 201) {
          Swal.fire({
            title: 'This Course has been added in you wish list',
            icon: 'success',
            toast: true,
            timer: 3000,
            position: 'top-right',
  ...
  const removeFavorite = (pk) => {
    ...
    try {
      axios.get(baseUrl + '/student-remove-favorite-course/' + course_id + '/' + studentId, {
        ...
        if (res.status === 200 || res.status === 201) {
          Swal.fire({
            title: 'This Course has been removed in you wish list',
            icon: 'success',
            toast: true,
            timer: 3000,
            position: 'top-right',
        ...
        <div className=" flex p-7 justify-left ml-32">
          ...
          <div className="mx-5 leading-8">
            ...
            }
            {
              ...
              <button className="font-bold py-2 px-4 rounded outline outline-1 text-red-700 ml-2 hover:bg-red-400" type="button" onClick={markAsFav}><i class="fa-regular fa-heart text-red-700 "></i>
            ...
            {
              userLoginStatus === 'success' && favoriteStatus === 'success' &&
              <button className="font-bold py-2 px-4 rounded outline outline-1 text-red-700 ml-2 bg-red-400" type="button" onClick={removeFavorite}><i class="fa-regular fa-heart text-red-700 "></i>
              ...
              <div className="overflow-hidden border rounded-lg">
                <table className="min-w-full divide-y divide-gray-200 ">
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {chapterData.map((chapter, index) =>
                      <tr key={index}>
                        <td className="px-6 py-4 text-sm text-gray-800  flex justify-between">
                          ...
                          <div className="flex">
                          {chapter.material  &&
                            <a href={chapter.material} target="_blank" rel="noopener noreferrer" className="mr-4 mt-1 hover:text-blue-700">
                            Click me to Download
                            <i class="fa-solid fa-file ml-4"></i> 
                            </a>
                          }
                          {chapter.material === null && 
                            <p className="mr-4 mt-1">No file available</p> 
                          }
                            {/* Modal */}
                            <div className="">
                              <button
                    ...
                    <p className="m-5 text-center font-bold text-2xl">Sorry the teacher has not uploaded any data yet.</p>
